...
parser.add_argument("-c", "--code", help="Code comune", required=False, default=None)
...
if vars(args)['input'] == '':
    ...
    while example_answer not in ['y', 'n']:
        ...
        if example_answer == 'y':
            import os
            input_path = context.projectpath() / "data/example.nc"
...
if code == None:
    ...
    w = processed_weather_forecasts.copy()
    mod = load_model(context.projectpath() / f'models/model_Lombardia_nospand')
    load_config(context.projectpath() / f'models/config_Lombardia_nospand')

    borders = get_borders_comuni()
    lomb = borders.dissolve().boundary

    w['pred'] = predict_model(mod, w.drop(columns='date')).rename(columns={'Label': 'pred'})[['pred']]
    gdf = gpd.GeoDataFrame(w, geometry=gpd.points_from_xy(w.lon, w.lat, crs=4326))
    gdf['geometry'] = gdf['geometry'].buffer(0.05, cap_style=3)
    gdf['date'] = gdf.date.dt.strftime('%b %d')
    gs = gdf.groupby('date')

    npanels = gdf.date.nunique()
    ncols = int(np.ceil((npanels**0.5)))
    nrows = int(npanels % ncols)

    fig, axs = plt.subplots(nrows=nrows, ncols=ncols, figsize=(8,8), sharex=True, sharey=True, dpi=150)
    plt.subplots_adjust(wspace=0.1, hspace=0.1)
    cbar = False
    vmin = gdf.pred.min()
    vmax = gdf.pred.max()
    axs = axs.ravel()
    for i, (ax, (date, g)) in enumerate(list(zip(axs[:npanels], gs))):
        if i+1 == npanels:
            cbar = True
        g.plot(ax=ax, column='pred', cmap='viridis', legend=cbar, vmin=vmin, vmax=vmax,
            legend_kwds={"label": "Predicted PM$_{2.5}$, $\mu g/m^3$", "orientation": "vertical", "shrink":0.8})
        lomb.plot(ax=ax, color='black')
        ax.set_title(date)
        ax.grid(False)
        ax.set_axis_off()
    axs[-1].axis('off')
